...
function Exam06() {
    //state가 배열인 경우
    const [numbers, setNumbers] = useState([10, 20, 20, 40, 50]);

    const [more, setMore] = useState("");
    const addNumber = useCallback((e)=>{
        //numbers.push(more); //쓰면 안되는 명령(원본을 직접 변경)

        //concat은 새로운 배열을 만들어내며, 이와 같은 신규 배열로 만들어 덮어쓰는게 리액트스러운 방법
            ...
            <div className="col">
                <ul className="list-group">
                    ...
                    <li className="list-group-item" key={index}>
                        ...
                        <FaXmark className="ms-2 text-danger" onClick={(e)=>deleteNumber(index)}/>
                    </li>
                    ))}
                </ul>
            </div>
