//Axios를 백엔드 인증 및 구조에 맞게 분할 및 개조하여 사용할 수 있도록 처리하여 제공하는 파일
...
//(추가) [3] API Axios객체의 요청이 실패한 상황 중 응답코드가 401번인 경우 갱신 요청
//- axios에는 interceptior라는 기능이 존재 axios의 각종상황에 들어가서 차단하는 역할
...
apiClient.interceptors.request.use(
    config=>{
        ...
        config.headers["X-Client-Page"] = origin + pathname;//파라미터제거
...
apiClient.interceptors.response.use(
  ...
  async function (error) {
    // console.log(Object.keys(error)); // error 객체의 모든 실패 정보
    ...
    // console.log(error.config); // 원래 하려고 하던 요청정보(에러가 나서 못함)
    const originalRequest = error.config;
    if(!originalRequest) {
        return Promise.reject(error);//통과
    }

    //최초 요청인 경우를 제외하고는 모두 통과시켜서 재요청이 발생하ㄷ지 않도록 한다
    ...
    try {
        ...
        //현재 화면은 로그인 상태이므로 갱신이 필요하지 않음(필요하다면 해도 됨)
        return apiClient(originalRequest);//apiClient에 원래 요청을 다시 보낸 결과를 반환
    }
    catch(refreshError) {
        //로그인 페이지로 강제이동(리액트스럽게는 어려움)
        moveToLoginPage();
        return Promise.reject(refreshError); //새로운 에러로 갈아끼우고 통과시켜라
    }
  }
);
