...
import { FaChevronDown, FaMagnifyingGlass, FaPlus, FaTrash } from "react-icons/fa6"
...
export default function CountryComplexSearch() {
    ...
    const changeStringValue = useCallback((e)=>{
        ...
        setCondition(prev=>({ //연관항목없이 쓰는 방법
            ...prev,
            [name] : value
        }));
    }, []);
    ...
    //state는 변경이 가능하므로 변경 불가능한건 memo로 생성할 수도 있따
    ...
    const availableOrderList = useMemo(()=>{
        //기존 : 같은 요소만 제거
        // return orderList.filte(order=>!condition.orders.includes(order)); 
    ...
    const changeOrder = useCallback((e, index)=>{
        ...
        const clone = [...condition.orders]; //원본은 한번에 바꿔야 하기 때문에 영향 없도록 복제
        const curr = clone[index];//현재 적용중인 값
        
        //같으면 중지
        if(dest === curr) return;
        
        //(+추가) 같은 항목을 오름차순/내림차순 동시 추가하는 것을 차단
        ...
        //예상 시나리오
        //1. 변경값(dest)이 선택항목(clone)에 없으면 해당 위치만 변경하면 됨
        ...
        //2. 변경값(dest)이 선택항목(clone)에 있으면 해당 위치와 현재 위치의 값을 바꿈
        else {
    ...
    const changeToKorean = useCallback((eng)=>{
        ...
        if(eng === "country_capital asc") return "수도 오름차순";
        if(eng === "country_capital desc") return "수도 내림차순";
        if(eng === "country_population asc") return "인구 적은 순";
    ...
    return(<>
        ...
        <Row className="mt-4">
            ...
            <Col sm={9}>
                <Form.Select type="text" value={condition.size}
                    name="size" onChange={changeNumericValue}>
