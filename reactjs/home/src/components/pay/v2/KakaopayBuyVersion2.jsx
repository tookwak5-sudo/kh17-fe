...
import { Button, Col, Form, Row, ListGroup, ListGroupItem } from "react-bootstrap";
...
/*
    ...
    기존에 만든 상품 페이지에서 구매를 누르면 이쪽의 해당 상품을 전달해야 한다.
    
    [1] 서버에 임시 결제 예정 정보를 저장하고 해당 번호만 전달하여 불러오는 방법
    (ex)    1번 상품 3개
            2번 상품 5개 => 주문번호 7번  
    (장점) 
        1. 번호 하나로 무수히 많은 결제정보를 저장하고 불러올 수 있다
        2. 결제가 진행중인 경우를 기억해서 나중에 알려줄 수 있다
    (단점) 
        1. DB 테이블이 필요하다
        2. 추가 관리 로직이 필요하다
...
export default function KakaopayBuyVersion2() {
    ...
    const loadData = useCallback(async () => {
        ...
        //?sale1:5&sale3:2&sale5:1 /상품번호 1번 5개, 3번 2개, 5번 1개
        const params = searchParams.getAll("sale")
            .map(str => {
                //:을 기준으로 분해해서 앞이 saleNo, 뒤가 quantity인 형태의 객체로 변환
                // const arr = str.split(":"); //이러면 saleNO[0] quantity[0]
        ...
        // - params를 key=value 형태로 바꾸고 data.daleList를 순회하며 saleNo와 매칭되는 수량을 찾아서 추가
                            ...
                            <div className="d-flex">
                                ...
                                <div className="text-end">
                                    {order.saleOriginalPrice > order.saleDiscountPrice ? (<>
                                        ...
                                        <b>{order.saleOriginalPrice.toLocaleString()}</b>
                                    </>)}
                                </div>
                            </div>
                        </ListGroupItem>
