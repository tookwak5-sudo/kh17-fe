...
export default function AccountCart() {
    ...
    const calculateDiscountRate = useCallback((item) => {
        if (item.original <= item.discount) return 0;
        if (item.discount === 0) return 100;
        const discount = item.original - item.discount;
        const rate = discount * 100 / item.original;
    ...
    const changeItemQty = useCallback((e, target) => {
        ...
        setCartList(
            prev => prev.map(
                item => {
                    if (item.no === target.no) { //내가 찾는 상품이면
    ...
    const totalAmount = useMemo(() => {
        //return cartList.reduce((acc, cur) => { //(계산함수, 초기값);
        return cartList.reduce((acc, cur) => {
            if (cur.choice === true) { //체크되어있다면
                return acc + cur.discount * cur.qty; //할인가을 합산
            }
            return acc;//아니면 그대로 반환
        }, 0);
    }, [cartList]);

    // [2] 체크된 상품의 할인 전/후 금액을 각각 구하여 하단에 출력 (similar like Gmarket)

    const totalAmountObject = useMemo(() => {
        return cartList.reduce(
            (acc, cur) => { //acc가 객체 (original, discount라는 필드가 존재)
                if (cur.choice === true) {//체크되어 있다면
                    return {
                        original: acc.original + cur.original * cur.qty,
                        discount: acc.discount + cur.discount * cur.qty,
                    }
                }
                return acc;//아니면 그대로 반환
            },
            { original: 0, discount: 0 } //초기값
                            ...
                            <div className="d-flex align-items-center">
                                ...
                                <div className="ms-4  flex-grow-1">
                                    ...
                                    <div className="text-start">
                                        {item.original > item.discount ? (<>
                                            <s className="text-muted">{item.original.toLocaleString()} 원</s>
                                            <br />
                                            <b className="text-danger">{item.discount.toLocaleString()} 원</b>
                                            <span className="test-success">
                                                <FaArrowTrendDown style={{ transform: "rotate(55deg)" }} />
                                                {calculateDiscountRate(item)} %
                                            </span>
                                        </>) : (<>
                                            <b>{item.original.toLocaleString()}</b>
                                        </>)}
                                    </div>
                                </div>
                ...
                <Row className="mt-5 text-end fs-3">
                    <Col>
                        <div className="d-flex justify-content-between">
                            <span>판매금액</span>
                            <span>{totalAmountObject.original.toLocaleString()}원</span>
                        </div>
                        <div className="d-flex justify-content-between">
                            <span>할인금액</span>
